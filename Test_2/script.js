    ...
    productcontainer.innerHTML += `
      <div class="product">
          ...
          <div class="priceandaddtocart">
              ...
              <button class="addtocart" productid=${e.id} onclick="addToCart(${e.id})">add to cart icon</button>
          </div>
      </div>
  `;
});
...
class Storage {
    ...
    static removeproduct(id) {
        let products = Storage.getproducts();
        products = products.filter((p) => p.id !== id); // Use filter to remove the product with the matching id
        localStorage.setItem("products", JSON.stringify(products));
    }
}
...
function addToCart(productId) {
    ...
    const newProduct = new Product(product.id, product.title, product.price, product.image);
    Storage.addtolocalstorage(newProduct);
    bag.setAttribute("items", Storage.getproducts().length);
}